  ...
  return (
    ...
    >
      ...
      <div style={{ padding: '32px 48px 32px 48px', display: 'flex', flexDirection: 'column', height: '100%', boxSizing: 'border-box', position: 'relative', zIndex: 1 }}>
        {/* 上部分：照片 + 基本信息 */}
        <div style={{ display: 'flex', gap: '36px', paddingBottom: '16px', marginBottom: '0', alignItems: 'flex-start', flex: '1 0 55%', overflow: 'hidden' }}>
          {/* 照片区域 - 证件照尺寸比例 (3:4) */}
          ...
          <div style={{ flex: 1, paddingTop: '66px', paddingLeft: '132px' }}>
            ...
            <div style={{ display: 'grid', rowGap: '18px' }}>
              ...
              {employee.systemJoinDate && (
                <div style={{ fontSize: '20px', lineHeight: '28px', color: 'rgb(255,255,255)', marginTop: '-8px', opacity: 0.9, fontFamily: 'Noto Sans SC, sans-serif', paddingLeft: '170px' }}>
        ...
        {/* 下部分：工作职责/信条 + 荣誉 */}
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', columnGap: '36px', flex: '1 0 45%', marginTop: '-40px', minHeight: 0, overflow: 'hidden' }}>
          {/* 左侧：工作职责 + 工作信条 */}
              ...
              {isLoadingDetail && !fallbackHonors ? (
                ...
                (selectedEmployeeDetail?.honors?.length > 0 ? selectedEmployeeDetail.honors : fallbackHonors).map((honor: any, idx: number) => (
                  <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: '15px', maxWidth: '100%' }}>
                    <span style={{ fontSize: '24px', lineHeight: '42px', fontWeight: 400, color: 'rgb(255,255,255)', whiteSpace: 'normal', display: 'block', width: '100%', wordBreak: 'keep-all', overflowWrap: 'break-word' }}>
                      {honor.title}
                    </span>
                  </div>
                ))
        ...
        <div className="h-20 flex items-center justify-between px-3">
          ...
          <div className="absolute left-1/2 transform -translate-x-1/2 text-center">
            <h1 className="showcase-title" style={{ fontFamily: 'Microsoft YaHei, 微软雅黑, sans-serif' }}>
              {activeStrategy?.displayMode === 'core_bones' ? '骨干展示墙' : activeStrategy?.displayMode === 'company_showcase' ? '公司展示墙' : '员工展示墙'}
            </h1>
          </div>
